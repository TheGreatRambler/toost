...
namespace ImGui {
	...
	IMGUI_API bool DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components,
		float v_speed = 1.0f, const void* p_min = NULL, const void* p_max = NULL, const char* format = NULL,
		ImGuiSliderFlags flags = 0);
	...
	IMGUI_API bool IsMouseClicked(
		ImGuiMouseButton button, bool repeat = false);       // did mouse button clicked? (went from !Down to Down)
...
enum ImGuiTableFlags_ {
	...
	= 1 << 12, // [ALPHA] Disable vertical borders in columns Body until hovered for resize (borders will always appears
...
enum ImGuiTableColumnFlags_ {
	...
	= 1
	  << 30 // [Internal] Disable user resizing this column directly (it may however we resized indirectly from its left
			// edge)
...
enum ImDrawFlags_ {
	...
	= 1 << 5, // AddRect(), AddRectFilled(), PathRect(): enable rounding top-right corner only (when rounding > 0.0f, we
			  // default to all corners). Was 0x02.
...
struct ImDrawList {
	...
	IMGUI_API void AddRect(const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f,
		ImDrawFlags flags = 0, float thickness = 1.0f); // a: upper-left, b: lower-right (== upper-left + size)
...
enum ImFontAtlasFlags_ {
	...
	= 1
	  << 2 // Don't build thick line textures into the atlas (save a little texture memory). The AntiAliasedLinesUseTex
...
namespace ImGui {
	...
	// static inline ImDrawList* GetOverlayDrawList()            { return GetForegroundDrawList(); } // OBSOLETED
	// in 1.69 (from Mar 2019) static inline void  SetScrollHere(float ratio = 0.5f)     { SetScrollHereY(ratio); } //
	...
	// inline void  ShowTestWindow()                      { return ShowDemoWindow(); }                                //
	// OBSOLETED in 1.53 (between Oct 2017 and Dec 2017) static inline bool  IsRootWindowFocused()                 {
	// return IsWindowFocused(ImGuiFocusedFlags_RootWindow); }   // OBSOLETED in 1.53 (between Oct 2017 and Dec 2017)
	...
	// return GetFrameHeightWithSpacing(); }                     // OBSOLETED in 1.53 (between Oct 2017 and Dec 2017)
}
...
enum ImDrawCornerFlags_ {
	...
	= ImDrawFlags_RoundCornersNone, // Was == 0 prior to 1.82, this is now == ImDrawFlags_RoundCornersNone which is != 0
									// and not implicit
