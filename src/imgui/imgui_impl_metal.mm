...
//  2021-05-19: Metal: Replaced direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become
//  a requirement) 2021-02-18: Metal: Change blending equation to preserve alpha in output buffer. 2021-01-25: Metal:
//  Fixed texture storage mode when building on Mac Catalyst. 2019-05-29: Metal: Added support for large mesh (64K+
//  vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: Metal: Added support for special
//  ImDrawCallback_ResetRenderState callback to reset render state. 2019-02-11: Metal: Projecting clipping rectangles
//  correctly using draw_data->FramebufferScale to allow multi-viewports for retina display. 2018-11-30: Misc: Setting
//  up io.BackendRendererName so it can be displayed in the About Window. 2018-07-05: Metal: Added new Metal backend
//  implementation.
...
// #import <QuartzCore/CAMetalLayer.h> // Not supported in XCode 9.2. Maybe a macro to detect the SDK version can be
// used (something like #if MACOS_SDK >= 10.13 ...)
#import <simd/simd.h>
...
@property (nonatomic, strong)
	FramebufferDescriptor* framebufferDescriptor; // framebuffer descriptor for current frame; transient
@property (nonatomic, strong)
	NSMutableDictionary* renderPipelineStateCache; // pipeline cache; keyed on framebuffer descriptors
...
void ImGui_ImplMetal_RenderDrawData(
	ImDrawData* draw_data, id<MTLCommandBuffer> commandBuffer, id<MTLRenderCommandEncoder> commandEncoder) {
...
// However, using a shader designed for 1-channel texture would make it less obvious to use the ImTextureID facility to
// render users own textures. You can make that change in your implementation.
- (void)makeFontTextureWithDevice:(id<MTLDevice>)device {
	ImGuiIO& io = ImGui::GetIO();
...
- (_Nullable id<MTLRenderPipelineState>)renderPipelineStateForFrameAndDevice:(id<MTLDevice>)device {
	...
	if(renderPipelineState == nil) {
		...
		renderPipelineState = [self _renderPipelineStateForFramebufferDescriptor:self.framebufferDescriptor
																		  device:device];
...
- (id<MTLRenderPipelineState>)_renderPipelineStateForFramebufferDescriptor:(FramebufferDescriptor*)descriptor
	...
	for(int n = 0; n < drawData->CmdListsCount; n++) {
		...
		for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if(pcmd->UserCallback) {
				...
				[commandEncoder setVertexBufferOffset:(vertexBufferOffset + pcmd->VtxOffset * sizeof(ImDrawVert))
											  atIndex:0];
