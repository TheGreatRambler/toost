...
// [ALPHA] Early backend, not well tested. If you want a portable application, prefer using the GLFW or SDL platform
// Backends on Mac.

// Implemented features:
//  [X] Platform: Mouse cursor shape and visibility. Disable with 'io.ConfigFlags |=
//  ImGuiConfigFlags_NoMouseCursorChange'. [X] Platform: OSX clipboard is supported within core Dear ImGui (no specific
//  code in this backend).
// Issues:
...
//  NSApplicationDidBecomeActiveNotification/NSApplicationDidResignActiveNotification events. 2021-06-23: Inputs: Added
//  a fix for shortcuts using CTRL key instead of CMD key. 2021-04-19: Inputs: Added a fix for keys remaining stuck in
//  pressed state when CMD-tabbing into different application. 2021-01-27: Inputs: Added a fix for mouse position not
//  being reported when mouse buttons other than left one are down. 2020-10-28: Inputs: Added a fix for handling
//  keypad-enter key. 2020-05-25: Inputs: Added a fix for missing trackpad clicks when done with "soft tap". 2019-12-05:
//  Inputs: Added support for ImGuiMouseCursor_NotAllowed mouse cursor. 2019-10-11: Inputs:  Fix using Backspace key.
//  2019-07-21: Re-added clipboard handlers as they are not enabled by default in core imgui.cpp (reverted 2019-05-18
//  change). 2019-05-28: Inputs: Added mouse cursor shape and visibility support. 2019-05-18: Misc: Removed clipboard
//  handlers as they are now supported by core imgui.cpp. 2019-05-11: Inputs: Don't filter character values before
//  calling AddInputCharacter() apart from 0xF700..0xFFFF range. 2018-11-30: Misc: Setting up io.BackendPlatformName so
//  it can be displayed in the About Window. 2018-07-07: Initial version.
...
bool ImGui_ImplOSX_Init() {
	...
	// (optional, rarely used) io.BackendFlags |= ImGuiBackendFlags_PlatformHasViewports;    // We can create
	// multi-viewports on the Platform side (optional) io.BackendFlags |= ImGuiBackendFlags_HasMouseHoveredViewport; //
	...
	// by adding '#define IMGUI_ENABLE_OSX_DEFAULT_CLIPBOARD_FUNCTIONS' in imconfig.h and adding '-framework
	// ApplicationServices' to your linker command-line. Since we are already in ObjC land here, it is easy for us to
