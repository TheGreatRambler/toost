...
//  g_XXXX access changed to bd->XXXX). 2021-05-19: OpenGL: Replaced direct access to ImDrawCmd::TextureId with a call
//  to ImDrawCmd::GetTexID(). (will become a requirement) 2021-01-03: OpenGL: Backup, setup and restore GL_SHADE_MODEL
//  state, disable GL_STENCIL_TEST and disable GL_NORMAL_ARRAY client state to increase compatibility with legacy OpenGL
//  applications. 2020-01-23: OpenGL: Backup, setup and restore GL_TEXTURE_ENV to increase compatibility with legacy
//  OpenGL applications. 2019-04-30: OpenGL: Added support for special ImDrawCallback_ResetRenderState callback to reset
//  render state. 2019-02-11: OpenGL: Projecting clipping rectangles correctly using draw_data->FramebufferScale to
//  allow multi-viewports for retina display. 2018-11-30: Misc: Setting up io.BackendRendererName so it can be displayed
//  in the About Window. 2018-08-03: OpenGL: Disabling/restoring GL_LIGHTING and GL_COLOR_MATERIAL to increase
...
void ImGui_ImplOpenGL2_RenderDrawData(ImDrawData* draw_data) {
	...
	for(int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		glVertexPointer(
			2, GL_FLOAT, sizeof(ImDrawVert), (const GLvoid*)((const char*)vtx_buffer + IM_OFFSETOF(ImDrawVert, pos)));
		glTexCoordPointer(
			2, GL_FLOAT, sizeof(ImDrawVert), (const GLvoid*)((const char*)vtx_buffer + IM_OFFSETOF(ImDrawVert, uv)));
