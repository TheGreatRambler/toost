...
//  g_XXXX access changed to bd->XXXX). 2021-05-19: Renderer: Replaced direct access to ImDrawCmd::TextureId with a call
//  to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: Change blending equation to preserve alpha in
//  output buffer. 2020-08-10: Inputs: Fixed horizontal mouse wheel direction. 2019-12-05: Inputs: Added support for
...
//  rectangle. 2018-06-13: Renderer: Backup/restore transform and clipping rectangle. 2018-06-11: Misc: Setup
//  io.BackendFlags ImGuiBackendFlags_HasMouseCursors flag + honor ImGuiConfigFlags_NoMouseCursorChange flag.
//  2018-04-18: Misc: Renamed file from imgui_impl_a5.cpp to imgui_impl_allegro5.cpp.
//  2018-04-18: Misc: Added support for 32-bit vertex indices to avoid conversion at runtime. Added imconfig_allegro5.h
//  to enforce 32-bit indices when included from imgui.h. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback
//  and exposed ImGui_ImplAllegro5_RenderDrawData() in the .h file so you can call it yourself. 2018-02-06: Misc:
...
#define ALLEGRO_HAS_CLIPBOARD                                                                                          \
	(ALLEGRO_VERSION_INT >= ((5 << 24) | (1 << 16) | (12 << 8))) // Clipboard only supported from Allegro 5.1.12
...
static void ImGui_ImplAllegro5_SetupRenderState(ImDrawData* draw_data) {
	...
	al_set_separate_blender(
		ALLEGRO_ADD, ALLEGRO_ALPHA, ALLEGRO_INVERSE_ALPHA, ALLEGRO_ADD, ALLEGRO_ONE, ALLEGRO_INVERSE_ALPHA);
...
void ImGui_ImplAllegro5_RenderDrawData(ImDrawData* draw_data) {
	...
	for(int n = 0; n < draw_data->CmdListsCount; n++) {
		...
		// Allegro's implementation of al_draw_indexed_prim() for DX9 is completely broken. Unindex our buffers
		// ourselves.
		// FIXME-OPT: Unfortunately Allegro doesn't support 32-bit packed colors so we have to convert them to 4 float
		// as well..
		...
		if(sizeof(ImDrawIdx) == 2) {
			// FIXME-OPT: Unfortunately Allegro doesn't support 16-bit indices.. You can '#define ImDrawIdx int' in
			// imconfig.h to request Dear ImGui to output 32-bit indices. Otherwise, we convert them from 16-bit to
			// 32-bit at runtime here, which works perfectly but is a little wasteful.
		...
		for(int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if(pcmd->UserCallback) {
				...
				al_draw_prim(&vertices[0], bd->VertexDecl, texture, idx_offset, idx_offset + pcmd->ElemCount,
...
bool ImGui_ImplAllegro5_Init(ALLEGRO_DISPLAY* display) {
	...
	ALLEGRO_VERTEX_ELEMENT elems[]
		= { { ALLEGRO_PRIM_POSITION, ALLEGRO_PRIM_FLOAT_2, IM_OFFSETOF(ImDrawVertAllegro, pos) },
			  ...
			  { ALLEGRO_PRIM_COLOR_ATTR, 0, IM_OFFSETOF(ImDrawVertAllegro, col) }, { 0, 0, 0 } };
