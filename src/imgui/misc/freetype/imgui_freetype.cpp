...
namespace {
	...
	struct FontInfo {
		...
		float LineSpacing;    // The baseline-to-baseline distance. Note that it usually is larger than the sum of the
						   // ascender and descender taken as absolute values. There is also no guarantee that no glyphs
						   // extend above or below subsequent baselines when using this distance. Think of it as a
	...
	struct FreeTypeFont {
		...
		void BlitGlyph(
			const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch, unsigned char* multiply_table = NULL);
		~FreeTypeFont() {
			CloseFont();
		}
...
#define FT_CEIL(X) (((X + 63) & -64) / 64)
	...
	void FreeTypeFont::SetPixelHeight(int pixel_height) {
		// Vuhdo: I'm not sure how to deal with font sizes properly. As far as I understand, currently ImGui assumes
		// that the 'pixel_height' is a maximum height of an any given glyph, i.e. it's the sum of font's ascender and
		// descender. Seems strange to me. NB: FT_Set_Pixel_Sizes() doesn't seem to get us the same result.
	...
	const FT_Glyph_Metrics* FreeTypeFont::LoadGlyph(uint32_t codepoint) {
		...
		// Apply convenience transform (this is not picking from real "Bold"/"Italic" fonts! Merely applying FreeType
		// helper transform. Oblique == Slanting)
