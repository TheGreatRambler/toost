...
// then encoded in Base85 to fit in a string so we can fit roughly 4 bytes of compressed data into 5 bytes of source
// code (suggested by @mmalex) (If we used 32-bit constants it would require take 11 bytes of source code to encode 4
// bytes, and be endianness dependent) Note that even with compression, the output array is likely to be bigger than the
// binary file.. Load compressed TTF fonts with ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF()
...
static bool binary_to_compressed_c(
	const char* filename, const char* symbol, bool use_base85_encoding, bool use_compression);
