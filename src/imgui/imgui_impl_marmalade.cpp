...
//  2021-05-19: Renderer: Replaced direct access to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will
//  become a requirement) 2019-07-21: Inputs: Added mapping for ImGuiKey_KeyPadEnter. 2019-05-11: Inputs: Don't filter
//  value from character callback before calling AddInputCharacter(). 2018-11-30: Misc: Setting up
//  io.BackendPlatformName/io.BackendRendererName so they can be displayed in the About Window. 2018-02-16: Misc:
//  Obsoleted the io.RenderDrawListsFn callback and exposed ImGui_Marmalade_RenderDrawData() in the .h file so you can
//  call it yourself. 2018-02-06: Misc: Removed call to ImGui::Shutdown() which is not available from 1.60 WIP, user
//  needs to call CreateContext/DestroyContext themselves. 2018-02-06: Inputs: Added mapping for ImGuiKey_Space.
...
bool ImGui_Marmalade_Init(bool install_callbacks) {
	...
	io.KeyMap[ImGuiKey_Tab] = s3eKeyTab io.KeyMap[ImGuiKey_LeftArrow] = s3eKeyLeft;
...
void ImGui_Marmalade_NewFrame() {
	...
	// For retina display or other situations where window coordinates are different from framebuffer coordinates. User
	...
	io.MousePos = ImVec2((float)mouse_x / g_scale.x,
		(float)mouse_y / g_scale.y); // Mouse position (set to -FLT_MAX,-FLT_MAX if no mouse / on another screen, etc.)

	for(int i = 0; i < 3; i++) {
		io.MouseDown[i]
			= g_MousePressed[i]
			  || s3ePointerGetState((s3ePointerButton)i)
					 != S3E_POINTER_STATE_UP; // If a mouse press event came, always pass it as "mouse held this frame",
