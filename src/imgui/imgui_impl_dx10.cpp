...
//  g_XXXX access changed to bd->XXXX). 2021-05-19: DirectX10: Replaced direct access to ImDrawCmd::TextureId with a
//  call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-02-18: DirectX10: Change blending equation to
//  preserve alpha in output buffer. 2019-07-21: DirectX10: Backup, clear and restore Geometry Shader is any is bound
//  when calling ImGui_ImplDX10_RenderDrawData(). 2019-05-29: DirectX10: Added support for large mesh (64K+ vertices),
//  enable ImGuiBackendFlags_RendererHasVtxOffset flag. 2019-04-30: DirectX10: Added support for special
...
bool ImGui_ImplDX10_CreateDeviceObjects() {
	...
	// Create the vertex shader
	{
		...
		if(FAILED(D3DCompile(
			   vertexShader, strlen(vertexShader), NULL, NULL, NULL, "main", "vs_4_0", 0, 0, &vertexShaderBlob, NULL)))
		...
		if(bd->pd3dDevice->CreateVertexShader(
			   vertexShaderBlob->GetBufferPointer(), vertexShaderBlob->GetBufferSize(), &bd->pVertexShader)
			!= S_OK) {
	...
	// Create the pixel shader
	{
		...
		if(FAILED(D3DCompile(
			   pixelShader, strlen(pixelShader), NULL, NULL, NULL, "main", "ps_4_0", 0, 0, &pixelShaderBlob, NULL)))
			return false; // NB: Pass ID3DBlob* pErrorBlob to D3DCompile() to get error showing in (const
						  // char*)pErrorBlob->GetBufferPointer(). Make sure to Release() the blob!
		if(bd->pd3dDevice->CreatePixelShader(
			   pixelShaderBlob->GetBufferPointer(), pixelShaderBlob->GetBufferSize(), &bd->pPixelShader)
			!= S_OK) {
