...
//  g_XXXX access changed to bd->XXXX). 2021-03-22: Vulkan: Fix mapped memory validation error when buffer sizes are not
//  multiple of VkPhysicalDeviceLimits::nonCoherentAtomSize. 2021-02-18: Vulkan: Change blending equation to preserve
//  alpha in output buffer. 2021-01-27: Vulkan: Added support for custom function load and
...
static void ImGui_ImplVulkan_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline,
	VkCommandBuffer command_buffer, ImGui_ImplVulkanH_FrameRenderBuffers* rb, int fb_width, int fb_height) {
	ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
